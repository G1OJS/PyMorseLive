...
class CorrelDecoder:
    ...
    def gen_patterns(self):
        import numpy as np
        dt = self.spec['dt']
        self.patterns = np.zeros((len(self.C2M), self.spec['pgrid'].shape[1]))
        bpd = 3
        for i, syms in enumerate(self.C2M.values()):
            pat1_bipolar = 2*np.array(self.encode_syms(syms,bits_per_dit = bpd)) - 1
            lth = len(pat1_bipolar)            
            self.patterns[i,-lth:] = pat1_bipolar / lth
...
def test():
    ...
    while True:
        ...
        ltr = list(d.C2M.keys())[np.argmax(correl)]
        print(ltr)
        """
        if cplot is None:
